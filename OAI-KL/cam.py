...
if args.model_type == 'densenet_161':
    model_ft.load_state_dict(torch.load('./Grad CAM/Image Size Original Revision/1_DenseNet-161.pt'))
    target_layers = [model_ft.features[-1]]
    image_size_tuple = (224, 224)
    
elif args.model_type == 'efficientnet_b5':
    model_ft.load_state_dict(torch.load('./Grad CAM/Image Size Original Revision/2_EfficientNet-b5.pt'))
    target_layers = [model_ft.features[-1]]
    image_size_tuple = (224, 224)
    
elif args.model_type == 'efficientnet_v2_s':
    model_ft.load_state_dict(torch.load('./Grad CAM/Image Size Original Revision/3_EfficientNet-V2-s.pt'))
    target_layers = [model_ft.features[-1]]
    image_size_tuple = (224, 224)
    
elif args.model_type == 'regnet_y_8gf':
    model_ft.load_state_dict(torch.load('./Grad CAM/Image Size Original Revision/4_RegNet-Y-8GF.pt'))
    target_layers = [model_ft.trunk_output[-1]]
    image_size_tuple = (224, 224)
    
elif args.model_type == 'resnet_101':
    model_ft.load_state_dict(torch.load('./Grad CAM/Image Size Original Revision/5_ResNet-101.pt'))
    target_layers = [model_ft.layer4[-1]]
    image_size_tuple = (224, 224)
    
elif args.model_type == 'resnext_50_32x4d':
    model_ft.load_state_dict(torch.load('./Grad CAM/Image Size Original Revision/6_ResNext-50-32x4d.pt'))
    target_layers = [model_ft.layer4[-1]]
    image_size_tuple = (224, 224)
    
elif args.model_type == 'wide_resnet_50_2':
    model_ft.load_state_dict(torch.load('./Grad CAM/Image Size Original Revision/7_Wide-ResNet-50-2.pt'))
    target_layers = [model_ft.layer4[-1]]
    image_size_tuple = (224, 224)
    
elif args.model_type == 'shufflenet_v2_x2_0':
    model_ft.load_state_dict(torch.load('./Grad CAM/Image Size Original Revision/8_ShuffleNet-V2-x2-0.pt'))
