...
for i in submission_list_csv:
    ...
    for k in range(1656):
        globals()['{}_image_{}'.format(i, k)] = [0, 0, 0, 0, 0]
    ...
    for k in range(5):
        ...
        submission_probs = submission_csv['prob_{}'.format(k)]
        submission_probs_list = submission_probs.values.tolist()
        
        for j in range(1656):
            globals()['{}_image_{}'.format(i, j)][k] = submission_probs_list[j] 
            
    for k in range(1656):       
        globals()['{}_probs'.format(i)].append(globals()['{}_image_{}'.format(i, k)])
